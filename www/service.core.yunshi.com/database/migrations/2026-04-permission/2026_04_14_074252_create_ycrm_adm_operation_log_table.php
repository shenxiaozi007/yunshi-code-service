...
return new class extends Migration
{
    ...
    {
        Schema::create('ycrm_adm_operation_log', function (Blueprint $table) {
            $table->comment('CRM-操作日志');
            $table->increments('id');
            $table->string('operation_log_no', 64)->default('')->comment('用户编号')->unique();
            $table->string('account_no', 64)->default('')->comment('账户no')->index();
            $table->string('action', 32)->default('')->comment('动作');
            $table->string('ip', 64)->default('')->comment('登录ip')->index();
    ...
    {
        Schema::dropIfExists('ycrm_adm_operation_log');
    }
};
