...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('ycode_adm_account_bind_role', function (Blueprint $table) {
            ...
            $table->string('account_no', 64)->default('')->comment('后端管理员no')->index();
            $table->string('role_no', 64)->default('')->comment('角色no')->index();
            $table->string('encryption_option', 32)->default('')->comment('选择的加密选项');
            $table->string('add_adm_no', 64)->default('')->comment('添加此角色的管理员id');
            $table->string('add_adm_name', 64)->default('')->comment('添加此角色的管理员真实姓名');
            $table->string('edit_adm_no', 64)->default('')->comment('最后编辑此管理员的id');
            $table->string('edit_adm_name', 64)->default('')->comment('最后编辑此角色的管理员真实姓名');
